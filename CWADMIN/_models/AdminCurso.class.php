...
class AdminCurso {
    ...
    /**
     * <b>Cadastrar Curso:</b> Envelopa os dados em um array atribuitivo e execute este método
    ...
    public function ExeCreate(array $Data) {
        ...
        if ($this->Data['foto']):
            $upload = new Upload;
            $upload->Image($this->Data['foto'], $this->Data['titulo'], 480, 'cursos');
        endif;
     ...
     * curso para atualiza-lo no sistema!
    ...
    /**
     * <b>Remover Curso:</b> Informe o ID do curso que deseja remover.
     * @param INT $CursoId = Id do curso
     */
    public function ExeDelete($CursoId) {
        ...
        if (!$readVideo->getResult()):
            $this->Error = ['Oppsss, você tentou remover um curso que não existe no sistema!', WS_ERROR];
            $this->Result = false;
        else:
            $this->fotoDelete($this->Id);
            $this->Delete();
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            $this->Error = ["O curso <b>{$this->Data['curso']}</b> foi cadastrada com sucesso no sistema!", WS_ACCEPT];
    ...
    private function Update() {
        ...
        if ($Update->getResult()):
            $this->Error = ["O curso <b>{$this->Data['curso']}</b> foi atualizada com sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
    ...
    private function Delete() {
        ...
        if ($Delete->getResult()):
            $this->Error = ["Curso removido com sucesso do sistema!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
